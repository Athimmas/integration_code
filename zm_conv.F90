...
!
   ...
   !dir$ attributes offload:mic :: rl
   real(r8) rl         ! wg latent heat of vaporization.
   !dir$ attributes offload:mic :: cpres
   ...
   !dir$ attributes offload:mic :: tfreez
   real(r8) :: tfreez
   !dir$ attributes offload:mic :: eps1
   real(r8) :: eps1
...
!moved from moistconvection.F90
   ...
   !dir$ attributes offload:mic :: rgas
...
!
!-----------------------------------------------------------------------
   use wv_sat_methods, only : default_idx,tboil,tmelt,epsilo
   implicit none
!-----------------------------------------------------------------------
!
! input arguments
...
!
   ...
   real(r8) rhd
#endif
   real*8, save :: totalTime = 0._r8
   real*8 :: start_time, end_time
   real*8 :: loopCurrentTime
   real*8, save :: timeForLoop
!
!-----------------------------------------------------------------------
!
...
!
   ...
   do k = pver,msg + 1,-1
      do i = 1,ncol
         hmn(i) = cp*t(i,k) + grav*z(i,k) + rl_copy*q(i,k)
!
! Reset max moist static energy level when relative difference exceeds 1.e-4
!
         rhd = (hmn(i) - hmax(i))/(hmn(i) + hmax(i))
...
!! Offloading to the Phi  Boo yeah \m/

   !!dir$ offload begin target(mic : 1) in(lchnk, ncol, msg, mx, p, t, q, tpert, cpwv, rh2o,rgas,tfreez,cpliq,eps1,cpres,tboil,default_idx,tmelt,rl,latice,grav,epsilo,iam) inout(tp, tpv, qstp, pl, tl, lcl,loopCurrentTime)
   call parcel_dilute(lchnk, ncol, msg, mx, p, t, q, tpert, tp, tpv, qstp, pl, tl, lcl, loopCurrentTime)
   !!dir$ end offload

   if(loopCurrentTime.lt.1._r8) then
      timeForLoop = timeForLoop + loopCurrentTime 
   endif

   open(27, file="getDeepConvectionTime", status="unknown")
   if(iam.eq.0) then
       write(27,*) "Time for Deep Convection loop", timeForLoop
   endif
...
!!DIR$ ATTRIBUTES OFFLOAD : mic :: parcel_dilute
subroutine parcel_dilute (lchnk, ncol, msg, klaunch, p, t, q, tpert, tp, tpv, qstp, pl, tl, lcl,loopCurrentTime)
...
real*8, intent(inout) :: loopCurrentTime !Loop Timings on the Phi
...
integer i,k,ii,jj,kk   ! Loop counters.

real*8 :: totalLoopStartTime,totalLoopEndTime ! Timing loops
...
totalLoopEndTime = omp_get_wtime()
loopCurrentTime = loopCurrentTime + (totalLoopEndTime - totalLoopStartTime)
...
!-----------------------------------------------------------------------------------------
!DIR$ ATTRIBUTES OFFLOAD : mic :: entropy
...
!
!-----------------------------------------------------------------------------------------
   !DIR$ ATTRIBUTES OFFLOAD : mic :: ientropy
...
! 
     use wv_sat_methods, only : wv_sat_dummySubroutine
...
converge: do i=0, LOOPMAX
   ...
   !Commenting the lines below out to see if loop vectorizes
   if (abs(dTs).lt.0.001_r8) then
       !write(iulog,*) 'Final iteration ', i
       exit converge
   endif
...
!DIR$ ATTRIBUTES OFFLOAD : mic :: qmmr_hPa
