...
!===============================================================================
  ...
  subroutine compute_uwshcu_inv( mix      , mkx        , iend          , ncnst     , dt       ,  & 
    ...
    use wv_saturation, only : pcf, estbl, plenest, omeps, c3, omeps_wv_saturation => omeps
    use physconst, only : tmelt,rh2o, epsilo_physconst => epsilo
    use modal_aero_data, only : numptr_amode
    use wv_sat_methods, only : epsilo_wv_sat_methods => epsilo, omeps_wv_sat_methods => omeps
    use constituents,    only : qmin, cnst_name, cnst_type
   ...
   !!dir$ offload begin target(mic : 1) in(lchnk,mix,mkx,iend,ncnst,dt,ps0,zs0,p0,z0,dp0,dpdry0,u0,v0,qv0,ql0,qi0,t0,s0,tr0,tke,cldfrct,concldfrct,pblh,pcf,estbl,tmelt,iulog,xlv,xlf,xls,cp,zvir,r,g,ep2,p00,rovcp,rpen,numptr_amode,plenest,c3,epsilo_physconst,epsilo_wv_sat_methods,rh2o,omeps_wv_saturation,omeps_wv_sat_methods,qmin,cnst_name,cnst_type) inout(cush,timeForShallowConvectionLoop,timeForEarlyExits,timeForLateExits) out(umf,qvten,qlten,qiten,sten,uten,vten,trten,qrten,qsten,precip,snow,evapc,slflx,qtflx,flxprc1,flxsnow1,cufrc,qcu,qlu,qiu,cbmf,qc,rliq,cnt,cnb)
  ...
  !!DIR$ ATTRIBUTES OFFLOAD : mic :: compute_uwshcu
  subroutine compute_uwshcu( mix      , mkx       , iend         , ncnst    , dt        , &
                             ...
                             cnt_out  , cnb_out   , lchnk        , dpdry0_in, timeForShallowConvectionLoop, timeForEarlyExits, timeForLateExits )
...
#endif
    use wv_saturation,   only : findsp_vc, findsp_vc_mod
    ...
    real(r8) :: t1_start_time, t1_end_time, t2_start_time, t2_end_time, t3_start_time, t3_end_time
...
!   parameter (rkm = 16.0_r8)        !  Determine the amount of air that is involved in buoyancy-sorting [no unit] 
    ...
    qtsrc_out(i)             = 0.0_r8
    thlsrc_out(i)            = 0.0_r8
    thvlsrc_out(i)           = 0.0_r8
    emfkbup_out(i)           = 0.0_r8
    cbmflimit_out(i)         = 0.0_r8
    tkeavg_out(i)            = 0.0_r8
    zinv_out(i)              = 0.0_r8
    rcwp_out(i)              = 0.0_r8
    rlwp_out(i)              = 0.0_r8
    ...
    exit_klclmkx(i)          = 0.0_r8 
    exit_klfcmkx(i)          = 0.0_r8 
    exit_ufrc(i)             = 0.0_r8 
    exit_wtw(i)              = 0.0_r8 
    exit_drycore(i)          = 0.0_r8 
    exit_wu(i)               = 0.0_r8 
    exit_cufilter(i)         = 0.0_r8 
    exit_kinv1(i)            = 0.0_r8 
    exit_rei(i)              = 0.0_r8 

    limit_shcu(i)            = 0.0_r8 
    limit_negcon(i)          = 0.0_r8 
    limit_ufrc(i)            = 0.0_r8
    limit_ppen(i)            = 0.0_r8
    limit_emf(i)             = 0.0_r8
    limit_cinlcl(i)          = 0.0_r8
    limit_cin(i)             = 0.0_r8
    limit_cbmf(i)            = 0.0_r8
    limit_rei(i)             = 0.0_r8
    ...
    do k = 1, mkx
       call findsp_vc_mod(qv0_in(i,k), t0_in(i,k), p0_in(i,k), .true., &
...
!the indices are not reversed, these variables go into compute_mcshallow_inv, this is why they are called "flxprc1" and "flxsnow1". 
 ...
 333 if(id_exit) then ! Exit without cumulus convection
  ...
  !DIR$ ATTRIBUTES OFFLOAD : mic :: getbuoy
  ...
  !DIR$ ATTRIBUTES OFFLOAD : mic :: single_cin
  ...
  !DIR$ ATTRIBUTES OFFLOAD : mic :: conden
  ...
  !DIR$ ATTRIBUTES OFFLOAD : mic :: roots
  ...
  !DIR$ ATTRIBUTES OFFLOAD : mic :: slope
  ...
  !DIR$ ATTRIBUTES OFFLOAD : mic :: qsinvert
  ...
  !DIR$ ATTRIBUTES OFFLOAD : mic :: compute_alpha
  ...
  !DIR$ ATTRIBUTES OFFLOAD : mic :: compute_mumin2
 ...
 20 return
  ...
  !DIR$ ATTRIBUTES OFFLOAD : mic :: compute_ppen
  ...
  !DIR$ ATTRIBUTES OFFLOAD : mic :: fluxbelowinv
  ...
  !DIR$ ATTRIBUTES OFFLOAD : mic :: positive_moisture_single
