...
subroutine phys_run1(phys_state, ztodt, phys_tend, pbuf2d,  cam_in, cam_out)
    ...
    if ( adiabatic .or. ideal_phys )then
       ...
       !Pushing the chunk level loop inside
       call tphysbc (ztodt, pbuf2d, fsns, fsnt, flns, flnt, phys_state(begchunk:endchunk),        &
                       phys_tend(begchunk:endchunk), fsds, landm,          &
                       sgh30, cam_out(begchunk:endchunk), cam_in(begchunk:endchunk) )
...
subroutine tphysbc (ztodt, pbuf2d,              &
    ...
    use cam_diagnostics, only: diag_conv_tend_ini, diag_phys_writeout, diag_conv, diag_export, diag_state_b4_phys_write, diag_physvar_ic
    use cam_history,     only: outfld
    ...
    real(r8), intent(inout) :: fsns(pcols,begchunk:endchunk)                   ! Surface solar absorbed flux
    real(r8), intent(inout) :: fsnt(pcols,begchunk:endchunk)                   ! Net column abs solar flux at model top
    real(r8), intent(inout) :: flns(pcols,begchunk:endchunk)                   ! Srf longwave cooling (up-down) flux
    real(r8), intent(inout) :: flnt(pcols,begchunk:endchunk)                   ! Net outgoing lw flux at model top
    real(r8), intent(inout) :: fsds(pcols,begchunk:endchunk)                   ! Surface solar down flux
    real(r8), intent(in) :: landm(pcols,begchunk:endchunk)                   ! land fraction ramp
    ...
    type(physics_ptend)   :: ptend(begchunk:endchunk)            ! indivdual parameterization tendencies - Buffer
    ...
    real(r8) :: zdu(begchunk:endchunk,pcols,pver)               ! detraining mass flux from deep convection - Buffer
    real(r8) :: cmfmc(begchunk:endchunk,pcols,pverp)            ! Convective mass flux--m sub c - Buffer

    real(r8) cmfcme(pcols,pver)                ! cmf condensation - evaporation
    real(r8) cmfmc2(begchunk:endchunk,pcols,pverp)               ! Moist convection cloud mass flux - Buffer
    real(r8) dlf(begchunk:endchunk,pcols,pver)                   ! Detraining cld H20 from shallow + deep convections - Buffer
    real(r8) dlf2(begchunk:endchunk,pcols,pver)                  ! Detraining cld H20 from shallow convections - Buffer
    ...
    real(r8),pointer :: prec_str(:)    ! sfc flux of precip from stratiform (m/s) - Buffer
    real(r8),pointer :: snow_str(:)     ! sfc flux of snow from stratiform   (m/s) - Buffer
    ...
    real(r8),pointer :: prec_sed(:)     ! total precip from cloud sedimentation - Buffer
    real(r8),pointer :: snow_sed(:)     ! snow from cloud ice sedimentation

    ! energy checking variables
    real(r8) :: zero(pcols)                    ! array of zeros
    real(r8) :: rliq(begchunk:endchunk,pcols)                    ! vertical integral of liquid not yet in q(ixcldliq) - Buffer
    ...
    type(check_tracers_data):: tracerint(begchunk:endchunk)             ! energy integrals and cummulative boundary fluxes - Buffer
    real(r8) :: zero_tracers(pcols,pcnst)

    logical   :: lq(pcnst)

    !  pass macro to micro
    character(len=16) :: microp_scheme(begchunk:endchunk) ! - Buffer
    character(len=16) :: macrop_scheme(begchunk:endchunk) ! - Buffer

    ! Debug physics_state.
    logical :: state_debug_checks

    integer :: c
    real(r8), save :: timeForConvection
    real(r8) :: start_time, end_time 
    
  !$omp parallel do private(c,pbuf,state_debug_checks,lq,zero_tracers,flx_heat,flx_cnd,det_ice,det_s,rliq2,zero,snow_sed,prec_sed,snow_pcw,prec_pcw,snow_str,snow_sed_carma,prec_sed_carma,snow_sh,prec_sh,snow_dp,prec_dp,fracis,dtcore,cldiceini,cldliqini,qini,tini,teout,cld,itim,ifld,ixcldice,ixcldliq,i,k,m,lchnk,ierr,ncol,nstep,net_flx,cmfcme,pflx,rtdt,prec_str) firstprivate(state_sc,ptend_sc,ptend_aero,tend_sc) default(shared) num_threads(16)
  do c=begchunk, endchunk
    !
    ...
    call phys_getopts( microp_scheme_out      = microp_scheme(c), &
                       macrop_scheme_out      = macrop_scheme(c), &
                       state_debug_checks_out = state_debug_checks)
    ...
    tend(c) %dTdt(:ncol,:pver)  = 0._r8
    tend(c) %dudt(:ncol,:pver)  = 0._r8
    tend(c) %dvdt(:ncol,:pver)  = 0._r8
    ...
    call qneg3_modalx1( &
         ...
         1, pcnst, qmin  ,state(c)%q, qneg3_worst_thresh_amode )
#else
    call qneg3('TPHYSBCb',lchnk  ,ncol    ,pcols   ,pver    , &
         1, pcnst, qmin  ,state(c)%q )
#endif

    ! Validate state coming from the dynamics.
    if (state_debug_checks) &
         call physics_state_check(state(c), name="before tphysbc (dycore?)")

    call clybry_fam_adj( ncol, lchnk, map2chm, state(c)%q, pbuf )
    ...
    if (state_debug_checks) &
         call physics_state_check(state(c), name="clybry_fam_adj")

    fracis (:ncol,:,1:pcnst) = 1._r8
    !
    ! Dump out "before physics" state
    !
    call diag_state_b4_phys_write (state(c))

    ! compute mass integrals of input tracers state
    call check_tracers_init(state(c), tracerint(c))
    ...
    if (dycore_is('LR')) then
       call check_energy_fix(state(c), ptend(c), nstep, flx_heat)
       call physics_update(state(c), ptend(c), ztodt, tend(c))
       call check_energy_chng(state(c), tend(c), "chkengyfix", nstep, ztodt, zero, zero, zero, flx_heat)
    end if
    ! Save state for convective tendency calculations.
    call diag_conv_tend_ini(state(c), pbuf)

    call cnst_get_ind('CLDLIQ', ixcldliq)
    call cnst_get_ind('CLDICE', ixcldice)
    qini     (:ncol,:pver) = state(c)%q(:ncol,:pver,       1)
    cldliqini(:ncol,:pver) = state(c)%q(:ncol,:pver,ixcldliq)
    cldiceini(:ncol,:pver) = state(c)%q(:ncol,:pver,ixcldice)
    ...
    if( nstep > pbuf_times-1 ) then
       do k = 1,pver
          dtcore(:ncol,k) = (tini(:ncol,k) - dtcore(:ncol,k))/(ztodt) + tend(c)%dTdt(:ncol,k)
       end do
    ...
    lq(:) = .FALSE.
    lq(1) = .TRUE.
    call physics_ptend_init(ptend(c), state(c)%psetcols, 'dadadj', ls=.true., lq=lq)
  ...
  !!$omp parallel do private(c,pbuf,state_debug_checks,lq,zero_tracers,flx_heat,flx_cnd,det_ice,det_s,rliq2,zero,snow_sed,prec_sed,snow_pcw,prec_pcw,snow_str,snow_sed_carma,prec_sed_carma,snow_sh,prec_sh,snow_dp,prec_dp,fracis,dtcore,cldiceini,cldliqini,qini,tini,teout,cld,itim,ifld,ixcldice,ixcldliq,i,k,m,lchnk,ierr,ncol,nstep,net_flx,cmfcme,pflx,rtdt,prec_str) firstprivate(state_sc,ptend_sc,ptend_aero,tend_sc) default(shared) num_threads(1)
  do c=begchunk, endchunk
    !
    ! Output physics terms to IC file
    !
    ...
    call convect_deep_tend(  &
         ...
         rliq(c,:),    &
         ztodt,   &
         state(c),   ptend(c), cam_in(c)%landfrac, pbuf) 
    ...
    flx_cnd(:ncol) = prec_dp(:ncol) + rliq(c,:ncol)
    call check_energy_chng(state(c), tend(c), "convect_deep", nstep, ztodt, zero, flx_cnd, snow_dp, zero)
    ...
    call convect_shallow_tend (ztodt   , cmfmc(c,:,:),  cmfmc2(c,:,:)  ,&
         dlf(c,:,:)        , dlf2(c,:,:)   ,  rliq(c,:)   , rliq2, & 
         state(c)      , ptend(c)  ,  pbuf)
    call t_stopf ('convect_shallow_tend')

    call physics_update(state(c), ptend(c), ztodt, tend(c))

    flx_cnd(:ncol) = prec_sh(:ncol) + rliq2(:ncol)
    call check_energy_chng(state(c), tend(c), "convect_shallow", nstep, ztodt, zero, flx_cnd, snow_sh, zero)

    call check_tracers_chng(state(c), tracerint(c), "convect_shallow", nstep, ztodt, zero_tracers)
    ...
    timeForConvection = timeForConvection + (end_time - start_time)

   if(iam.eq.0) then
        open(86, file="getConvectionTime", status="unknown")
        write(86, *), "Total Time for convection is ", timeForConvection
    endif
  ...
  do c=begchunk, endchunk
    ...
    call sslt_rebin_adv(pbuf,  state(c))
    ...
    if (carma_do_cldice .or. carma_do_cldliq) then
       call carma_timestep_tend(state(c), cam_in(c), cam_out(c), ptend(c), ztodt, pbuf, dlf=dlf(c,:,:), rliq=rliq(c,:), &
       ...
       if (carma_do_detrain) then
          call check_energy_chng(state(c), tend(c), "carma_tend", nstep, ztodt, zero, prec_str+rliq(c,:), snow_str+rliq(c,:), zero)
       else
          call check_energy_chng(state(c), tend(c), "carma_tend", nstep, ztodt, zero, prec_str, snow_str, zero)
       end if
    end if

    call t_stopf('carma_timestep_tend')

    if( microp_scheme(c) == 'RK' ) then
       ...
       call stratiform_tend(state(c), ptend(c), pbuf, ztodt, &
            cam_in(c)%icefrac, cam_in(c)%landfrac, cam_in(c)%ocnfrac, &
            landm(1,c), cam_in(c)%snowhland, & ! sediment
            dlf(c,:,:), dlf2(c,:,:), & ! detrain
            rliq(c,:)  , & ! check energy after detrain
            cmfmc(c,:,:),   cmfmc2(c,:,:), &
            cam_in(c)%ts,      cam_in(c)%sst,        zdu(c,:,:))

       call physics_update(state(c), ptend(c), ztodt, tend(c))
       call check_energy_chng(state(c), tend(c), "cldwat_tend", nstep, ztodt, zero, prec_str, snow_str, zero)

       call t_stopf('stratiform_tend')

    elseif( microp_scheme(c) == 'MG' ) then
       ...
       if (macrop_scheme(c) .ne. 'CLUBB_SGS') then

          call macrop_driver_tend(state(c), ptend(c), ztodt, &
               cam_in(c)%landfrac, cam_in(c)%ocnfrac, &
               cam_in(c)%snowhland, & ! sediment
               dlf(c,:,:), dlf2(c,:,:), & ! detrain
               cmfmc(c,:,:),   cmfmc2(c,:,:), &
               cam_in(c)%ts,      cam_in(c)%sst, zdu(c,:,:),  pbuf, &
               det_s, det_ice)

          !  Since we "added" the reserved liquid back in this routine, we need 
	  !    to account for it in the energy checker
          flx_cnd(:ncol) = -1._r8*rliq(c,:ncol) 
	  flx_heat(:ncol) = det_s(:ncol)
          
          call physics_update(state(c), ptend(c), ztodt, tend(c))
          call check_energy_chng(state(c), tend(c), "macrop_tend", nstep, ztodt, zero, flx_cnd, det_ice, flx_heat)
          ...
          call clubb_tend_cam(state(c),ptend(c),pbuf,1.0_r8*ztodt,&
             cmfmc(c,:,:), cmfmc2(c,:,:), cam_in(c), sgh30(1,c), dlf(c,:,:), det_s, det_ice)

          !  Since we "added" the reserved liquid back in this routine, we need 
	  !    to account for it in the energy checker
          flx_cnd(:ncol) = -1._r8*rliq(c,:ncol) 
	  flx_heat(:ncol) = cam_in(c)%shf(:ncol) + det_s(:ncol)

          !    Update physics tendencies and copy state to state_eq, because that is 
          !      input for microphysics              
          call physics_update(state(c), ptend(c), ztodt, tend(c))
          call check_energy_chng(state(c), tend(c), "clubb_tend", nstep, ztodt, cam_in(c)%lhf/latvap, flx_cnd, det_ice, flx_heat)
       ...
       call microp_aero_run(state(c), ptend_aero, ztodt, pbuf)
       call t_stopf('microp_aero_run')

       call t_startf('microp_tend')

       call microp_driver_tend( &
            state(c), ptend(c), ztodt, pbuf)

       ! combine aero and micro tendencies
       call physics_ptend_sum(ptend_aero, ptend(c), ncol)

       call physics_update(state(c), ptend(c), ztodt, tend(c))
       call check_energy_chng(state(c), tend(c), "microp_tend", nstep, ztodt, zero, prec_str, snow_str, zero)
    ...
    if ( .not. deep_scheme_does_scav_trans() ) then
       ...
       call aerosol_wet_intr (state(c), ptend(c), ztodt, pbuf,  cam_out(c), dlf(c,:,:))
       call physics_update(state(c), ptend(c), ztodt, tend(c))
       ...
       if (carma_do_wetdep) then
          ...
          call carma_wetdep_tend(state(c), ptend(c), ztodt, pbuf, dlf(c,:,:), cam_out(c))
          call physics_update(state(c), ptend(c), ztodt, tend(c))
          call t_stopf ('carma_wetdep_tend')
       end if

       call t_startf ('convect_deep_tend2')
       call convect_deep_tend_2( state(c),   ptend(c),  ztodt,  pbuf ) 
       call t_stopf ('convect_deep_tend2')

       call physics_update(state(c), ptend(c), ztodt, tend(c))

       ! check tracer integrals
       call check_tracers_chng(state(c), tracerint(c), "cmfmca", nstep, ztodt,  zero_tracers)
    ...
    call diag_phys_writeout(state(c), cam_out(c)%psl)
    call diag_conv(state(c), ztodt, pbuf)
    ...
    call cloud_diagnostics_calc(state(c), pbuf)
    ...
    call radiation_tend(state(c),ptend(c), pbuf, &
         cam_out(c), cam_in(c), &
         cam_in(c)%landfrac,landm(1,c),cam_in(c)%icefrac, cam_in(c)%snowhland, &
         fsns(1,c),    fsnt(1,c), flns(1,c),    flnt(1,c),  &
         fsds(1,c), net_flx)

    ! Set net flux used by spectral dycores
    do i=1,ncol
       tend(c)%flx_net(i) = net_flx(i)
    end do
    call physics_update(state(c), ptend(c), ztodt, tend(c))
    call check_energy_chng(state(c), tend(c), "radheat", nstep, ztodt, zero, zero, zero, net_flx)

    call t_stopf('radiation')

    ! Diagnose the location of the tropopause and its location to the history file(s).
    call t_startf('tropopause')
    call tropopause_output(state(c))
    call t_stopf('tropopause')

    ! Save atmospheric fields to force surface models
    call t_startf('cam_export')
    call cam_export (state(c),cam_out(c),pbuf)
    call t_stopf('cam_export')

    ! Write export state to history file
    call t_startf('diag_export')
    call diag_export(cam_out(c))
